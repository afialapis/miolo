...
const TodosList = ({authType, authenticated, todoList, addTodo, toggleTodo, removeTodo, checkLastHours, insertFakeTodo}) => {
    ...
    <div className="todos">
      <div className="todos-header">
        ...
        <div className="todos-add">
          ...
          <div className={`button--primary ${((authType!=='credentials' || authenticated) && (inputText.length>0)) ? '' : 'disabled'}`}
        ...
        : <ol className="todos-list"> 
            {todoList.map((todo) => 
              <li key={`todo_${todo.id}`} className={"todo"}>
                ...
                {(authType!=='credentials' || authenticated)
                 ? <>
