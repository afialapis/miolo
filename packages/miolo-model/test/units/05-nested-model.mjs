...
import { Flavour, FlavourList, Bread, Bakery } from '../models.mjs'
import { FLAVOUR_TYPES, BREAD_TYPES, BAKERY_DATA } from '../data.mjs'
...
test('MioloModel nested objects', async (t) => {
  ...
  await t.test('get_extra_data handles properties explicitly defined as instance getters that map to MioloModel', () => {
    const flavours = new FlavourList(FLAVOUR_TYPES)
    const breadData = BREAD_TYPES[0]
    const bread = new Bread(breadData, flavours)
