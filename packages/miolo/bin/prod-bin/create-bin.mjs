...
export default async function(appName, srvDest, srvExt) {
  ...
  const readSource = (f) => readFileSync(path.resolve(__my_dirname, '../run', f), {encoding:'utf8'})
  const writeDest = (f, content) => writeFileSync(path.join(pkgPath, srvDest, f), content, {encoding:'utf8',flag:'w'})

  const pidContent = readSource('pid.mjs')
  writeDest('pid.mjs', pidContent)

  let startContent = readSource('start.mjs')
  startContent = startContent.replace('(appName, srvDest, srvExt) {', `(appName= '${appName}', srvDest= '${srvDest}', srvExt= '${srvExt}') {`)
  ...
  restartContent = restartContent.replace('(appName, srvDest, srvExt) {', `(appName= '${appName}', srvDest= '${srvDest}', srvExt= '${srvExt}') {`)
  writeDest('restart.mjs', restartContent)
  
  copyFileSync(path.join(__my_dirname, 'run.mjs'), path.join(pkgPath, srvDest, 'run.mjs'))
  copyFileSync(path.join(__my_dirname, '../../src/config/.env'), path.join(pkgPath, srvDest, '.env'))
}