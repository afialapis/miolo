...
async function main() {
  ...
  try {
    ...
    const cliEntry = args['cli-entry'] || process.env.MIOLO_BUILD_CLIENT_ENTRY || './src/cli/entry-cli.jsx'
    ...
    const srvEntry= args['server-entry'] || process.env.MIOLO_BUILD_SERVER_ENTRY || './src/server/server.mjs'
    const configEntry = args['config-entry'] || process.env.MIOLO_BUILD_CONFIG_ENTRY || './src/server/config.mjs'
    ...
    switch (command) {
      ...
      case 'build':
        ...
        await buildHandler(appName, cliEntry, cliDest, cliSuffix, htmlFile, srvEntry, configEntry, srvDest, srvExt, ssrEntry, ssrDest)
        break
