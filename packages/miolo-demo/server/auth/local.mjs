...
const get_user_id = (user, done, ctx) => { 
  ctx.miolo.logger.debug(`[miolo-demo][local-auth] get_user_id() - name ${user?.name} - sessionId ${ctx.sessionId}`)
  const uid= user?.id
  if (uid!=undefined) {
...
const find_user_by_id = (id, done, ctx) => {
  ctx.miolo.logger.debug(`[miolo-demo][local-auth] find_user_by_id() - id ${id} - sessionId ${ctx.sessionId}`)
  ctx.miolo.db.get_connection().then((conn) => {
    return q_find_user_by_id(conn, id).then(user => {
      ctx.miolo.logger.debug('[miolo-demo][local-auth] find_user_by_id()', JSON.stringify(user))
...
const local_auth_user = (username, password, done, ctx) => {
  ...
  ctx.miolo.logger.debug(`[miolo-demo][local-auth] local_auth_user() - checking credentials for ${username} - sessionId ${ctx.sessionId}`)

  ctx.miolo.db.get_connection().then((conn) => {
    q_auth_user(conn, username, password).then(user => {
      ctx.miolo.logger.debug('[miolo-demo][local-auth] local_auth_user() - User logged in', JSON.stringify(user))
