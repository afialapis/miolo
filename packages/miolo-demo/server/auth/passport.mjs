import {find_user_by_id as q_find_user_by_id, 
        local_auth_user as q_local_auth_user} from '#server/db/users.mjs'        
...
const find_user_by_id = (id, done, ctx) => {
  ...
  q_find_user_by_id(ctx.miolo, id).then(user => {
    if (user==undefined) {
...
const local_auth_user = (username, password, done, ctx) => {
  ...
  q_local_auth_user(ctx.miolo, username, password).then( ( [user, msg] ) => {
    if (user==undefined) {
